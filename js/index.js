"use strict";
import { validateOnLoad, cookieFun, byId } from "./exports.js";
cookieFun();
if (!window.localStorage)
    alert("Your browser does not support local storage. get a new browser.");
byId("main-title").addEventListener("click", () => {
    // SECRET DEBUGS STUFF
...
if (window.location.search.includes("action=login")) {
    ...
    validateOnLoad().then(state => {
        if (state)
            document.getElementById("login")?.classList.add("hide-keep-layout");
    });
}
