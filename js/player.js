...
if (!window.localStorage.getItem("nag-dismissed")) {
    ...
    nag.addEventListener("click", e => {
        e.stopPropagation();
        cover.classList.remove("fill");
        if (e.composedPath().includes(byId("nag-dismiss-forever")))
            window.localStorage.setItem("nag-dismissed", "true");
...
class Data {
    constructor() {
        ...
        for (const [, elem] of Object.entries(this.functionButtons)) {
...
//swipe to next song
a.album_art.addEventListener("touchstart", function (e) {
    a.toucher.start = e.touches[0].clientX;
});
a.album_art.addEventListener("touchmove", function (e) {
...
//swipe to next song
a.album_art.addEventListener("touchend", function (e) {
...
a.functionButtons.remove.addEventListener("click", () => {
    if (a.contextType !== "playlist")
        return alert("Cannot remove song from " + a.contextType + " context");
...
Array.from(a.decide_menu.children).forEach(div => div.addEventListener("click", e => {
    a.menu_cover.classList.remove("fill");
    a.decide_menu.classList.add("hide");
        ...
        fetch(BASEURL + "me/player/volume?volume_percent=" + a.saved_volume, {
            ...
            if (res.ok) {
                ...
                if (a.volume == 0)
                    ...
                    a.volume_button.querySelector("img").src = "/media/volume_33.png";
                else if (a.volume < 66)
                    a.volume_button.querySelector("img").src = "/media/volume_66.png";
                else
                    a.volume_button.querySelector("img").src = "/media/volume_99.png";
            }
            else {
                console.log(res);
                res.json().then(json => (a.error.innerText = json.error.message));
...
a.volume_range.addEventListener("change", function () {
...
a.seek_range.addEventListener("change", function () {
...
async function handleInterval() {
    ...
    if (res.status == 503)
        return resetInterval(4); // spotify throws 503 now and then but does not provide a retry-after, fun
    if (res.status == 429)
        return resetInterval(10); // user deserves to be punished for spamming
    if (res.status == 204)
        return resetInterval(a.timeout > 30 ? 30 : a.timeout + 1); // paused and nothing's changed
    if (res.status == 401) {
        endInterval();
    ...
    if (!data)
        return resetInterval(a.timeout >= 10 ? 10 : a.timeout + 1);
    ...
    if (a.song_uri == data["item"]["uri"] && !a.album_art.src.startsWith("data:"))
        return;
    //select biggest image
    a.album_art.src = data["item"]["album"]["images"].reduce((a, b) => (a.width > b.width ? a : b)).url;
    a.song_name.innerText = data["item"]["name"];
    a.song_artist.innerText = data["item"]["artists"].map((artist /* todo Spotify.Artist */) => artist["name"]).join(", ");
    ...
    if (a.contextId && a.contextType.endsWith("playlist"))
        byId("q-link").href = "/playlists/?selected=" + a.contextId; // todo make easy to add many params (will make when needed)
    //scuffed but works
    ...
    if (!a.interval)
        resetInterval(2); // runs on app init if needed
}
            ...
            for (let i = 0; i < pixels.byteLength; i++) {
                ...
                if (NORMALIZE_DIR > 0)
                    ...
                    hexValue = "#" + ((1 << 24) +
                        ...
                        .toString(16).slice(1);
                if (modeMap[hexValue] == null)
                    modeMap[hexValue] = 1;
                else
                    modeMap[hexValue]++;
                if (modeMap[hexValue] > maxEl[1])
                    maxEl = [hexValue, modeMap[hexValue]];
            }
            return maxEl[0];
        })();
        document.querySelector("meta[name=theme-color]")?.setAttribute("content", color);
        console.log(`%cTotal Time to calculate theme color: %c${performance.now() - startTime}ms
%cTime to read canvas: %c${readCanvasTime}ms
%cResult: %c${color}`, "color: #eee", "color: lightBlue", "color: #eee", "color: lightBlue", "color: #eee", `color: ${color}; background: ${color[1] > "3" ? "" : "white"}`);
    });
}
